...
| Library scan | Yes | Yes | CLI `scan`; GUI scans on startup and via Refresh. Both read installed games, playtime, hidden state, and Steam collections. GUI Library presents scanned games as poster cards with quick-view pills (All, Cozy, Story-rich, Great on Deck, Short sessions), search, advanced filters (genre, tag, ProtonDB tier, Deck, Controller, Unplayed, HLTB min/max range, playtime min/max range), sort by (Installed+Playtime, Name, Playtime, HLTB, Rating, AppID with Desc toggle), pagination (48/page with a **Load more** button), game card overflow menu (•••: Recommend, Copy AppID, Open Steam Store), and an insights rail (Total, Installed, Unplayed, Hidden, Junk, Playtime, Matching, Backlog, Recent activity, Avg HLTB). While the library snapshot is being prepared in the background, a skeleton loader is shown and action buttons are disabled. Short sessions filters by HLTB main_story_seconds ≤ 120 min (falls back to IGDB time_to_beat when HLTB is absent). |
...
- [ ] **Library**: loads demo games; skeleton appears briefly then clears; poster cards show deterministic placeholder art; filters/search work (genre, tag, ProtonDB, Deck, Controller, Unplayed, HLTB range, playtime range); sort dropdown changes order; **Load more** adds 48 cards per click; game card overflow menu (•••) opens with Recommend, Copy AppID, Open Steam Store; insights rail shows correct counts including Backlog (visible + zero playtime), Recent activity (top 3 by last_played_unix with relative time), Avg HLTB.
...
- [ ] **Playlists**: create/save a manual playlist; hero shows cover, content type, creator, owned/unplayed, avg HLTB; left rail shows each stored playlist with 44×44 deterministic cover, name, description snippet, content type badge, game count (— for unresolved rule playlists), and generator badge (Discover/Dynamic/Mood); corrupt files show per-item error state; ID auto-generates from name (slugified) with real-time path-traversal validation; Games tab: search and Add/Remove games from library is primary, raw CSV in collapsed Advanced section; Rules tab: visual rule tree with remove buttons (Not child removal deletes the Not node), quick-add buttons, parameterized rule adders (Genre, Tag, HLTB max, ProtonDB tier, Playtime range with min≤max validation, Rating min with 0.0–5.0 validation), inline error messages, Advanced JSON toggle (invalid JSON preserved — no data loss on visual mutations); Match tab switches; right rail shows description, content type, game count, generator badge; share code and export controls work; sync to Steam collection requires dry-run confirmation.
