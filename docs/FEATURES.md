...
| Library scan | Yes | Yes | CLI `scan`; GUI scans on startup and via Refresh. Both read installed games, playtime, hidden state, and Steam collections. GUI Library presents scanned games as poster cards with quick-view pills (All, Cozy, Story-rich, Great on Deck, Short sessions), search, advanced filters (genre, ProtonDB tier, Deck, Unplayed), pagination (60/page), and an insights rail (Total, Installed, Unplayed, Hidden, Junk, Playtime, Matching). Short sessions filters by HLTB main_story_seconds ≤ 120 min (falls back to IGDB time_to_beat when HLTB is absent). |
...
| Junk preview | Yes | Yes | Evaluates Default, Strict, or Aggressive junk modes after hydrating cached external metadata into scanned game records. GUI opens Junk from the Library toolbar (`Junk…` panel), not a sidebar item (ADR-0006). Panel: mode → Preview → table with per-row selection checkboxes, bulk "Select all junk"/"Clear", summary rail (Evaluated, Candidates, Selected, Sel. junk), and apply/hide. |
| Junk apply to collection | Yes | Yes | Writes junk candidates (or selected subset) to a Steam collection after dry-run/confirmation and backup. GUI action lives in the Library Junk panel (**Apply N selected** or **Apply all to collection**). |
| Junk hide | Yes | Yes | Adds junk candidates (or selected subset) to Steam's hidden collection after dry-run/confirmation and backup. GUI action lives in the Library Junk panel (**Hide N selected** or **Hide all**). |
| Recommendations | Yes | Yes | Recommends from hydrated scanned game records by available minutes, count, installed-only, Deck mode, and optional seed AppID. GUI top-level view is labeled **Recommendations**; Session Planner card with seed picker dropdown, top-3 highlight cards with medal icons and match-percent badges, explanation rail (Results, Avg score, Top score, methodology), and full results list with score and reason codes. |
