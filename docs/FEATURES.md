...
| Library scan | Yes | Yes | CLI `scan`; GUI scans on startup and via Refresh. Both read installed games, playtime, hidden state, and Steam collections. GUI Library presents scanned games as poster cards with quick-view pills (All/Installed/Unplayed/Hidden/Junk), search, advanced filters (genre, ProtonDB tier, Deck, Unplayed), pagination (60/page), and an insights rail (Total, Installed, Unplayed, Hidden, Junk, Playtime, Matching). |
