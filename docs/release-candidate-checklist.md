...
The authoritative checksum is generated by `scripts/build-release.sh` at
`target/release-artifacts/vapourfly-0.1.0-source.tar.gz.sha256`.

The checksum value is not committed into this checklist because this file is
part of the source archive; embedding the archive checksum here would make the
archive checksum self-referential and stale as soon as the checklist changes.
